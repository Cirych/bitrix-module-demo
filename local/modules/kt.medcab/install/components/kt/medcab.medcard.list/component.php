...
$arFilter =(array(
	'select' => array('*'),
	'filter' => array(
		$arParams['FOR'] => $USER->GetID(),
		'<=WORK_EVENT.START_DATE' => , // link to ???
		'>=WORK_EVENT.START_DATE' => // month?
	),
	'order' => array('WORK_EVENT.START_DATE') //link to eventstable
));
$arResult = WorksTable::getList($arFilter)->fetchAll();
	...
	if ($bAllowAccess)
	{
		...
		if ($USER->GetID())
		{
			...
			else
			{
				...
				$arFilter["PROPERTY_".$arParams["ELEMENT_ASSOC_PROPERTY"]] = $arParams["CLIENT_ID"];
			}
